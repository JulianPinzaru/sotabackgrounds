...
import inference.legacy
from opensimplex import OpenSimplex

class ImageGeneratorConfig(AppConfig):
    default_auto_field = 'django.db.models.BigAutoField'
    name = 'image_generator'
    MODEL_PATH = pathlib.Path("inference/networks/universe1024/network-snapshot-001280.pkl")
    BERT_PRETRAINED_PATH = pathlib.Path("inference/networks/universe1024/")
...
@click.option('--easing', type=click.Choice(['linear', 'easeInOutQuad', 'bounceEaseOut']), default='linear',
              help='easing method', required=True)
...
def generate_images(ctx: click.Context, easing: str, interpolation: str, increment: Optional[float], network_pkl: str,
        process: str, random_seed: Optional[int], diameter: Optional[float], seeds: Optional[List[int]], space: str,
        fps: Optional[int], frames: Optional[int], truncation_psi: float, noise_mode: str, outdir: str,
        class_idx: Optional[int], projected_w: Optional[str], start: Optional[float], stop: Optional[float], ):
